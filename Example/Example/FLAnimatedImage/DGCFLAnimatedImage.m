...
@optional
- (void)debug_animatedImage:(DGCFLAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(DGCFLAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(DGCFLAnimatedImage *)animatedImage;
@end
#endif


@interface DGCFLAnimatedImage ()
...
// The actual type of the object is `DGCFLWeakProxy`.
@property (nonatomic, strong, readonly) DGCFLAnimatedImage *weakProxy;
...
@implementation DGCFLAnimatedImage
...
+ (void)initialize
{
    if (self == [DGCFLAnimatedImage class]) {
...
- (instancetype)init
{
    DGCFLAnimatedImage *_Nullable const animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        FLLog(FLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `DGCFLAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (self) {
        ...
        _weakProxy = (id)[DGCFLWeakProxy weakProxyForObject:self];
...
{
    DGCFLAnimatedImage *const animatedImage = [[DGCFLAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[DGCFLAnimatedImage class]]) {
        DGCFLAnimatedImage *const animatedImage = (DGCFLAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        FLLog(FLLogLevelError, @"`image` isn't of expected types `UIImage` or `DGCFLAnimatedImage`: %@", image);
    }
...
@implementation DGCFLAnimatedImage (Logging)
...
#pragma mark - DGCFLWeakProxy

@interface DGCFLWeakProxy ()
...
@implementation DGCFLWeakProxy

#pragma mark Life Cycle

// This is the designated creation method of an `DGCFLWeakProxy` and
// as a subclass of `NSProxy` it doesn't respond to or need `-init`.
+ (instancetype)weakProxyForObject:(id)targetObject
{
    DGCFLWeakProxy *weakProxy = [DGCFLWeakProxy alloc];
