...
#import "DGCFLAnimatedImageView.h"
...
//
//  An `DGCFLAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `DGCFLAnimatedImageView`.
...
//
@interface DGCFLAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `DGCFLAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `DGCFLAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface DGCFLAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [DGCFLAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface DGCFLWeakProxy : NSProxy
